...
// import type { TicketMessage, TicketSummary, ReplyRequest } from '../../../types/mail.type.ts';
...
//                 s.loading = false;
//                 const incoming = a.payload.content || [];
//
//                 // Upsert theo id
//                 const byId = new Map<number, TicketSummary>(s.items.map(t => [t.id, t]));
//                 for (const t of incoming) {
//                     const old = byId.get(t.id);
//                     byId.set(t.id, old ? { ...old, ...t } : t);
//                 }
//
//                 // Sắp xếp lại theo lastMessageAt (mới nhất lên đầu)
//                 s.items = Array.from(byId.values()).sort(
...
//
//                 if (s.selectedId == null && s.items.length) {
...
//                 s.error = null;
//             })
//             .addCase(fetchMoreTickets.fulfilled, (s, a) => {
//                 s.loading = false;
//                 const incoming = a.payload.content || [];
//
//                 const byId = new Map<number, TicketSummary>(s.items.map(t => [t.id, t]));
